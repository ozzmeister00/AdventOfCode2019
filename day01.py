...
class Day01Solver(ProblemSolver):
    def __init__(self):
        ...
        self.testDataPartOne = {"12":2,
                                "14":2,
                                "1969":654,
                                "100756":33583}

        self.testDataPartTwo = {'14': 2,
                                '1969': 966,
                                '100756': 50346}

    def ProcessInput(self, data=None):
        """
        Convert the input data string into a
        list of integers
    ...
    def getFuelRequiment(self, mass):
        """
        ...
        :param int mass: how much mass is in our module
        :return int: how much fuel is required to launch the module
        """
        return math.floor(mass / 3) - 2

    def SolvePartOne(self, data=None):
        """
        Calculate the total fuel requirement for our ship

        :param list data: the list of module masses
        :return int: the total fuel requirement for our given set of module masses
        """
        if not data:
            data = self.processed

        fuelRequirment = sum([self.getFuelRequiment(i) for i in data])

        return fuelRequirment

    def SolvePartTwo(self, data=None):
        """
        Calculate the REAL total fuel requirement of our ship

        :param list data:
        :return int: the real total fuel requirement of our given set of module masses
        """
        if not data:
            data = self.processed
        ...
        for mass in data:
            ...
            while moduleFuelRequirement > 0:
                moduleFuelRequirement = max(self.getFuelRequiment(moduleFuelRequirement), 0)
                print(moduleFuelRequirement)

                fuelRequirement += moduleFuelRequirement
