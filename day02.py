...
will find an opcode - either 1, 2, or 99. The opcode indicates what to do; for example,
...
Once the program has halted, its output is available at address 0, also just like before. Each time you try a pair of
 inputs, make sure you first reset the computer's memory to the values in the program (your puzzle input) -
...
def add(inData, inPosition):
    """
    opcode 1 adds two numbers and stores the result in a third position

    the first value is stored in in + 1
    the second value is stored in in + 2
    in + 3 is where the result should be stored

    :param inData: the intcode buffer
    :param inPosition: the position of our add command

    :return: the resulting buffer
    """
    a = inData[inData[inPosition + 1]]
    b = inData[inData[inPosition + 2]]
    c = a + b
    inData[inData[inPosition + 3]] = c

    return inData


def mul(inData, inPosition):
    """
    opcode 2 multiplies two numbers and stores the result in a third position
    ...
    :param inData: the intcode buffer
    :param inPosition: the position of our mul command
...
class DaySolver02(ProblemSolver):
    def __init__(self):
        super(DaySolver02, self).__init__(2)

        self.testDataPartOne = {'1,0,0,0,99': [2,0,0,0,99],
    ...
    def ProcessInput(self, data=None):
        """
        :param str data:string of integers, separated by commas
    ...
    def SolvePartOne(self, data=None):
        ...
        if not data:
            ...
            # per instructions, set the value of data[1] to 12 and data[2] to 2
            # to combat the 1202 error
            data[1] = 12
            data[2] = 2

            # indicate to the solver that we're testing on live data, and should return the
